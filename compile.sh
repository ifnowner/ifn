...
echo "compiling ifn core..."
find src/java/ifn/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "ifn core class files compiled successfully"

echo "compiling ifn desktop..."
find src/java/ifndesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "ifn desktop class files compiled successfully"
else
    echo "if javafx is not supported, ifn desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
