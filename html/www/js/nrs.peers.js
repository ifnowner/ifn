        ...
        NRS.sendRequest("getPeers+", {
			...
			if (response.peers && response.peers.length) {
				...
				for (var i = 0; i < response.peers.length; i++) {
					...
					rows += "<tr>";
					rows += "<td>";
					rows += (peer.state == 1 ? "<i class='fa fa-check-circle' style='color:#5cb85c' title='"+$.t("Connected")+"'></i>" : "<i class='fa fa-times-circle' style='color:#f0ad4e' title='"+$.t("Not Connected")+"'></i>");
