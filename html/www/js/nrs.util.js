...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_ifn_decimals) + offset;
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                value = NRS.formatAmount(new BigInteger(String(value))) + " IFN";
    ...
    NRS.generateToken = function(message, secretPhrase) {
        ...
        token = token.concat(tsb, converters.hexStringToByteArray(NRS.signBytes(converters.byteArrayToHexString(messageBytes), secretPhrase !== undefined ? converters.stringToHexString(secretPhrase) : undefined)));
