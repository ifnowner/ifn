...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^IFN\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^IFN\-/i.test(data.id)) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_ifn").html($.t("sell_asset_for_ifn", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_ifn").html($.t("buy_asset_with_ifn", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_ifn_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_ifn_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "ifn": NRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "ifn": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_ifn": totalWIN
            });
        } else {
        ...
        $("#asset_order_total").html(totalWIN + " IFN");
