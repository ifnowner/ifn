...
public interface Fee {
    ...
    Fee DEFAULT_FEE = new Fee.ConstantFee(Constants.ONE_IFN);
    ...
    abstract class SizeBasedFee implements Fee {
        ...
        public final long getFee(TransactionImpl transaction, Appendix appendage) {
            ...
            if (height <= Constants.NEW_FEE_CALCULATION_BLOCK) {
                constantFee = (this.constantFee < Constants.ONE_IFN ? Constants.ONE_IFN : this.constantFee)/Constants.REDUCTOR_FEE;
                ...
                return fee < Constants.ONE_IFN/Constants.REDUCTOR_FEE ? Constants.ONE_IFN/Constants.REDUCTOR_FEE : fee;
            }

            if (this.constantFee == 0) constantFee = 0;
            else constantFee = (this.constantFee < Constants.ONE_IFN ? Constants.ONE_IFN : this.constantFee)/Constants.REDUCTOR_FEE;
            if (this.feePerSize == 0) feePerSize = 0;
            else feePerSize = (this.feePerSize < Constants.ONE_IFN ? Constants.ONE_IFN : this.feePerSize)/Constants.REDUCTOR_FEE;

            fee = (Math.addExact(constantFee, Math.multiplyExact((long) (size / unitSize), feePerSize)));

            if (fee != 0) {
                fee = (fee < Constants.ONE_IFN ? Constants.ONE_IFN : fee)/ Constants.REDUCTOR_FEE;
            }
            return fee;
        }
