...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Ifn.getBooleanProperty("ifn.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Ifn.getIntProperty("ifn.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Ifn.getStringProperty("ifn.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Ifn.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Ifn.getEpochTime() + blacklistingPeriod);
