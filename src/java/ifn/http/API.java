...
import static ifn.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static ifn.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static ifn.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Ifn.getStringProperty("ifn.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Ifn.getIntProperty("ifn.maxAPIRecords");
    static final boolean enableAPIUPnP = Ifn.getBooleanProperty("ifn.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Ifn.getIntProperty("ifn.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Ifn.getBooleanProperty("ifn.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Ifn.getStringListProperty("ifn.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Ifn.getStringListProperty("ifn.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Ifn.getStringListProperty("ifn.allowedBotHosts");
        ...
        boolean enableAPIServer = Ifn.getBooleanProperty("ifn.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Ifn.getIntProperty("ifn.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Ifn.getIntProperty("ifn.apiServerSSLPort");
            final String host = Ifn.getStringProperty("ifn.apiServerHost");
            disableAdminPassword = Ifn.getBooleanProperty("ifn.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Ifn.getBooleanProperty("ifn.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Ifn.getUserHomeDir()).resolve(Paths.get(Ifn.getStringProperty("ifn.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Ifn.getStringProperty("ifn.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Ifn.getStringProperty("ifn.keyStoreType"));
                List<String> ciphers = Ifn.getStringListProperty("ifn.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Ifn.getStringProperty("ifn.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Ifn.getStringProperty("ifn.apiWelcomeFile")});
            }

            String javadocResourceBase = Ifn.getStringProperty("ifn.javadocResourceBase");
            ...
            //ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/ifn");
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/"+Constants.COIN_NAME.toLowerCase());
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Ifn.getIntProperty("ifn.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            //servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/ifn-proxy");
            ...
            if (!Ifn.getBooleanProperty("ifn.enableAPIServerGZIPFilter")) {
            	//gzipHandler.setExcludedPaths("/ifn", "/ifn-proxy");
                gzipHandler.setExcludedPaths("/"+Constants.COIN_NAME.toLowerCase(), "/"+Constants.COIN_NAME.toLowerCase()+"-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(ifn.peer.Peers.MIN_COMPRESS_SIZE);
            apiHandler.setGzipHandler(gzipHandler);

            if (Ifn.getBooleanProperty("ifn.enableApiTestUI")) {
            ...
            if (Ifn.getBooleanProperty("ifn.apiFrameOptionsSameOrigin")) {
