...
import static ifn.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static ifn.http.JSONResponses.INCORRECT_DEADLINE;
import static ifn.http.JSONResponses.INCORRECT_EC_BLOCK;
import static ifn.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static ifn.http.JSONResponses.INCORRECT_WHITELIST;
import static ifn.http.JSONResponses.MISSING_DEADLINE;
import static ifn.http.JSONResponses.MISSING_SECRET_PHRASE;
import static ifn.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Ifn.getBlockchain().getHeight() + 1,
                Ifn.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws IfnException {
        ...
        //Logger.logMessage("ecBlockId="+ecBlockId+" | ecBlockHeight="+ecBlockHeight+" | Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
        //ecBlockId = new BigInteger("429943628837330866").longValue();
        if (ecBlockId != 0 && ecBlockId != Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
        	//suse 10/03/2017
        	JSONObject response = new JSONObject();
            response.put("errorCode", 4);
            response.put("errorDescription", "Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
            return JSON.prepare(response);
        	//JSONStreamAware msg = incorrect("ecBlockId", "ecBlockId does not match the block id at ecBlockHeight");
        	//var message = JSONStreamAware (incorrect"Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight); 
        	//return 
            //return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Ifn.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Ifn.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (IfnException.NotYetEncryptedException ignore) {}
            ...
            if (broadcast) {
                Ifn.getTransactionProcessor().broadcast(transaction);
                response.put("broadcasted", true);
            } else {
                transaction.validate();
                response.put("broadcasted", false);
            }
        } catch (IfnException.NotYetEnabledException e) {
            return FEATURE_NOT_AVAILABLE;
        } catch (IfnException.InsufficientBalanceException e) {
            throw e;
