...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Ifn.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Ifn.getBlockchainProcessor().popOffTo(Ifn.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Ifn.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Ifn.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Ifn.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
