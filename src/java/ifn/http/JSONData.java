...
import ifn.Account;
import ifn.AccountLedger;
import ifn.AccountLedger.LedgerEntry;
import ifn.AccountRestrictions;
import ifn.Alias;
import ifn.Appendix;
import ifn.Asset;
import ifn.AssetDelete;
import ifn.AssetDividend;
import ifn.AssetTransfer;
import ifn.Attachment;
import ifn.Block;
import ifn.Constants;
import ifn.Currency;
import ifn.CurrencyExchangeOffer;
import ifn.CurrencyFounder;
import ifn.CurrencyTransfer;
import ifn.CurrencyType;
import ifn.DigitalGoodsStore;
import ifn.Exchange;
import ifn.ExchangeRequest;
import ifn.FundingMonitor;
import ifn.Generator;
import ifn.HoldingType;
import ifn.MonetarySystem;
import ifn.Ifn;
import ifn.Order;
import ifn.PhasingPoll;
import ifn.PhasingVote;
import ifn.Poll;
import ifn.PrunableMessage;
import ifn.Shuffler;
import ifn.Shuffling;
import ifn.ShufflingParticipant;
import ifn.TaggedData;
import ifn.Token;
import ifn.Trade;
import ifn.Transaction;
import ifn.Vote;
import ifn.VoteWeighting;
import ifn.crypto.Crypto;
import ifn.crypto.EncryptedData;
import ifn.db.DbIterator;
import ifn.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Ifn.getBlockchain().getHeight());
    }
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWIN", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_IFN));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.IFN) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Ifn.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Ifn.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Ifn.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Ifn.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Ifn.getBlockchain().getTransaction(entry.getEventId());
