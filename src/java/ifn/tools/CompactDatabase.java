 ...
 *
 *   java -cp "classes:lib/*:conf" ifn.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dwin.runtime.mode=desktop ifn.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Ifn properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "ifn.testDb" : "ifn.db";
        String dbType = Ifn.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Ifn.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Ifn.getDbDir(Ifn.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Ifn.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Ifn.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Ifn.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "ifn.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "ifn.mv.db");
            if (!dbFile.exists()) {
