...
/**
 * Handle logging for the Ifn node server
 */
public final class Logger {
     ...
     * The ifn/conf/logging-default.properties and ifn/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "ifn.util.IfnLogManager");
        if (!(LogManager.getLogManager() instanceof IfnLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("ifn.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Ifn.loadProperties(loggingProperties, "logging-default.properties", true);
                Ifn.loadProperties(loggingProperties, "logging.properties", false);
                Ifn.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(ifn.Ifn.class);
        enableStackTraces = Ifn.getBooleanProperty("ifn.enableStackTraces");
        enableLogTraceback = Ifn.getBooleanProperty("ifn.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof IfnLogManager) {
            ((IfnLogManager) LogManager.getLogManager()).ifnShutdown();
        }
    }
