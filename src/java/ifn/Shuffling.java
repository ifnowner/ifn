...
import ifn.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Ifn.getBooleanProperty("ifn.deleteFinishedShufflings");
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f " + Constants.COIN_NAME + " awarded to forger at height %d", ((double)fee) / Constants.ONE_IFN, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f " + Constants.COIN_NAME + " awarded to forger at height %d", ((double)fee) / Constants.ONE_IFN, block.getHeight());
        }
