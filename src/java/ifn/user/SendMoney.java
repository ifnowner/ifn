...
import static ifn.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IfnException.ValidationException, IOException {
        ...
        } else if (feeNQT < Constants.ONE_IFN || feeNQT > Constants.MAX_BALANCE_NQT) {
        ...
        if (account == null || Math.addExact(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = Ifn.newTransactionBuilder(user.getPublicKey(),
