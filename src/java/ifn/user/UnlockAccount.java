...
import static ifn.user.JSONResponses.LOCK_ACCOUNT;
...
public final class UnlockAccount extends UserServlet.UserRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        if (account == null) {
            ...
            try (DbIterator<? extends Transaction> transactions = Ifn.getTransactionProcessor().getAllUnconfirmedTransactions()) {
            ...
            int blockchainHeight = Ifn.getBlockchain().getLastBlock().getHeight();
            try (DbIterator<? extends Block> blockIterator = Ifn.getBlockchain().getBlocks(accountId, 0)) {
            ...
            try (DbIterator<? extends Transaction> transactionIterator = Ifn.getBlockchain().getTransactions(accountId, (byte) -1, (byte) -1, 0, false)) {
